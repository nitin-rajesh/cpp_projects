    ...
    for(int row=numberOfRows-1;row>=0;row--){
        ...
        if(!isAI){
        	...
        	for(int i=0;i<4;i++){
                ...
                if((board.pos[row][1]='K'||board.pos[row][1]=='k')&&(board.pos[row][2]='E'||board.pos[row][2]=='e')){
                        for(int i=0;i<4;i++){
                            board.pos[row][i]='*';
                        }
                        showAns=true;
